...
def reset_configs(module):
    """
    Method to reset vlan configurations.
    :param module: The Ansible module to fetch input parameters.
    """
    ...
    if not multiple_vlan:
        for eth in eth_list:
            cmd = 'ip link del eth-{}-1.1'.format(eth)
            execute_commands(module, cmd)

    else:
        for eth in eth_list:
            for subport in range(1, 5):
                cmd = 'ip link del eth-{}-1.{}'.format(eth, subport)
                execute_commands(module, cmd)
